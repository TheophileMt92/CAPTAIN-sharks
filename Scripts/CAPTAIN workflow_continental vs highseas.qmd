...
fishing_with_realm <- st_join(fishing_sf, meow_ecos %>% dplyr::select(REALM))
...
fishing_with_ecoregion <- st_join(fishing_sf, meow_ecos %>% dplyr::select(ECOREGION))
...
process_priority_data <- function(data, dataset_name) {
  ...
  # Join with REALM
  data_with_realm <- st_join(data_sf, meow_ecos %>% dplyr::select(REALM))
  
  # Join with ECOREGION
  data_with_ecoregion <- st_join(data_sf, meow_ecos %>% dplyr::select(ECOREGION, REALM))
...
iucn_with_realm <- st_join(iucn_sf, meow_ecos %>% dplyr::select(REALM))
