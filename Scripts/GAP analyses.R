...
# Function to process and results
...
summarize_results <- function(comparison_data, mpa_type) {
  cat(paste("\nSummary for", mpa_type, "MPAs:\n"))
  cat("Top 5 over-represented species:\n")
  print(comparison_data %>% arrange(desc(difference)) %>% select(species, actual_percentage, mean_percentage, difference, z_score) %>% head(5))
  
  cat("\nTop 5 under-represented species:\n")
  print(comparison_data %>% arrange(difference) %>% select(species, actual_percentage, mean_percentage, difference, z_score) %>% head(5))
  
  cat("\nOverall statistics:\n")
  print(summary(comparison_data$difference))
  
  over_represented <- mean(comparison_data$difference > 0, na.rm=T) * 100
  under_represented <- mean(comparison_data$difference < 0, na.rm=T) * 100
  
  cat("\nPercentage of species over-represented:", 
      round(over_represented, 2), "%\n")
  cat("Percentage of species under-represented:", 
      round(under_represented, 2), "%\n")
  cat("Percentage of species equally represented:", 
      round(100 - over_represented - under_represented, 2), "%\n")
}
...
significant_species <- function(comparison_data, mpa_type, z_threshold = 1.96) {
  ...
  cat(paste("\nSignificantly over-represented species in", mpa_type, "MPAs (p < 0.05):\n"))
  print(over_represented %>% select(species, actual_percentage, mean_percentage, difference, z_score))
  
  cat(paste("\nSignificantly under-represented species in", mpa_type, "MPAs (p < 0.05):\n"))
  print(under_represented %>% select(species, actual_percentage, mean_percentage, difference, z_score))
}
...
# Step 1: Clean column names in puvsp_marine to match species names in difference_sp
puvsp_marine_clean <- puvsp_marine 

# Step 2: Reshape puvsp_marine from wide to long format
puvsp_long <- puvsp_marine_clean %>%
...
# Step 3: Join with difference_sp to get the difference values for each species
combined_data <- puvsp_long %>%
  left_join(difference_sp, by = c("species" = "species"))

# Step 4: Group by grid cell (id, lon, lat) and calculate mean difference
mean_diff_per_cell <- combined_data %>%
  group_by(id, lon, lat) %>%
  summarise(mean_difference = mean(difference, na.rm = TRUE),
            sum_difference = sum(difference, na.rm = TRUE)) %>%
  ungroup()
...
# Step 2: Apply Z-score transformation to mean_difference and sum_difference
mean_diff_per_cell <- mean_diff_per_cell %>%
  mutate(mean_difference_z = scale(mean_difference),
         sum_difference_z = scale(sum_difference))

# Step 3: Plot Z-score transformed mean_difference with land in dark grey
# Plot Z-score transformed mean_difference with land in dark grey
g1 = ggplot() +
  geom_tile(data = mean_diff_per_cell, aes(x = lon, y = lat, fill = mean_difference)) +
  geom_sf(data = land, fill = "darkgrey", color = NA) +  # Add land in dark grey
  scale_fill_gradient2(low = "#2166ac", mid = "#f0f0f0", high = "#b2182b", midpoint = 0) +  # Blue-Light Grey-Red gradient
  ...
  labs(title = "Mean Species Difference per Grid Cell",
       x = "Longitude",
       y = "Latitude",
       fill = "Mean Difference")
g1

#Plot the actual percentage
# Load necessary libraries
library(dplyr)
library(tidyr)
library(ggplot2)
...
# Step 1: Clean column names in puvsp_marine to match species names in difference_sp
puvsp_marine_clean <- puvsp_marine  # No renaming needed if species names match
...
# Step 3: Join with difference_sp to get the actual percentage and difference values for each species
combined_data <- puvsp_long %>%
  left_join(difference_sp, by = c("species" = "species"))

# Step 4: Group by grid cell (id, lon, lat) and calculate mean actual_percentage and mean_difference
mean_actual_per_cell <- combined_data %>%
  group_by(id, lon, lat) %>%
  summarise(mean_actual_percentage = mean(actual_percentage, na.rm = TRUE),
            mean_difference = mean(difference, na.rm = TRUE)) %>%
  ungroup()

summary(mean_actual_per_cell)

# Step 5: Apply Z-score transformation to the mean actual percentage
mean_actual_per_cell <- mean_actual_per_cell %>%
  mutate(mean_actual_percentage_z = scale(mean_actual_percentage))

# Step 5: Cap the mean_actual_percentage at 30
mean_actual_per_cell <- mean_actual_per_cell %>%
  mutate(mean_actual_percentage_capped = pmin(mean_actual_percentage, 30))  # Cap values at 30

# Optional: Apply Z-score transformation to the capped mean actual percentage
mean_actual_per_cell <- mean_actual_per_cell %>%
  mutate(mean_actual_percentage_capped_z = scale(mean_actual_percentage_capped))  # Z-score of capped values

# Load land polygons from rnaturalearth for visualization
land <- ne_countries(scale = "medium", returnclass = "sf")

# Step 6: Plot the capped mean actual percentage with land in dark grey
g2 = ggplot() +
  geom_tile(data = mean_actual_per_cell, aes(x = lon, y = lat, fill = mean_actual_percentage_capped)) +
  geom_sf(data = land, fill = "darkgrey", color = NA) +  # Add land in dark grey
  scale_fill_gradient2(high = "#2166ac", mid = "#f0f0f0", low = "#b2182b", midpoint = 10) +  # Blue-Light Grey-Red gradient
  coord_sf(xlim = c(-180, 180), ylim = c(-90, 90), expand = FALSE) +  # Set global coordinates
  theme_minimal() +
  theme(
    legend.position = "bottom",            # Position the legend at the bottom
  ...
  labs(title = "Capped Mean Actual Percentage (Max 30) per Grid Cell",
       x = "Longitude",
       y = "Latitude",
       fill = "Actual % Coverage (Capped)")
g2
...
# Inspect the first few rows to verify the aggregation
head(mean_ses_per_cell)
...
# Step 5: Cap the mean_actual_percentage at 30
mean_actual_per_cell <- mean_actual_per_cell %>%
  mutate(mean_actual_percentage_capped = pmin(mean_actual_percentage, 10))  # Cap values at 30
...
g4 = ggplot() +
  geom_tile(data = mean_actual_per_cell, aes(x = lon, y = lat, fill = mean_actual_percentage_capped)) +
  geom_sf(data = land, fill = "darkgrey", color = NA) +  # Add land in dark grey
  scale_fill_gradient(low="white", high="darkblue") +  # Blue-Light Grey-Red gradient
  #scale_fill_gradient2(high = "#2166ac", mid = "#f0f0f0", low = "#b2182b", midpoint = 5) +  # Blue-Light Grey-Red gradient
  coord_sf(xlim = c(-180, 180), ylim = c(-90, 90), expand = FALSE) +  # Set global coordinates
  theme_minimal() +
  theme(
    legend.position = "bottom",            # Position the legend at the bottom
  ...
  labs(title = "Capped Mean Actual Percentage (Max 10) per Grid Cell",
       x = "Longitude",
       y = "Latitude",
       fill = "Actual % Coverage (Capped)")
g4
...
# Create the violin plot with IUCN colors, without outliers in boxplots and for all MPAs
violin_plot <- ggplot(results_IUCN_filtered, aes(x = factor(IUCN_status, levels = iucn_order), y = percentage_in_ALL_MPA, fill = IUCN_status, color = IUCN_status)) +
  geom_violin(trim = FALSE, alpha = 0.5) +
  geom_jitter(width = 0.1, size = 0.5, color = "black", alpha = 0.5, color = "darkgray") +
  geom_boxplot(width = 0.1, fill = "white", color = "black", outlier.shape = NA, alpha = 0.8) +  # Remove outliers and set color
  labs(
       x = "IUCN Status",
       y = "(%) Range within MPA") +
  theme_minimal() +
...
kruskal_result <- kruskal.test(sqrt(percentage_in_ALL_MPA) ~ IUCN_status, data = results_IUCN_filtered)
# Print Kruskal-Wallis test result
print(kruskal_result)

# Perform Dunn's test for pairwise comparisons
dunn_result <- dunn.test(sqrt(results_IUCN_filtered$percentage_in_ALL_MPA), 
...
hull_points <- results_coords[chull(results_coords$A1, results_coords$A2), ]

# Create the scatter plot
ggplot(results_coords, aes(x = A1, y = A2)) +
  # Add the convex hull
  geom_polygon(data = hull_points, aes(x = A1, y = A2), fill = NA, color = "black", size = 0.5) +
  # Add the points
  geom_point(aes(size = percentage_in_ALL_MPA, color = percentage_in_ALL_MPA), alpha = 0.7) +
  scale_size_continuous(range = c(1, 10), name = "% in ALL MPA") +
  scale_color_viridis(option = "plasma", name = "% in ALL MPA") +
  theme_minimal() +
  labs(x = "PCoA Axis 1",
       y = "PCoA Axis 2") +
  theme(legend.position = "right",
        ...
        axis.text = element_text(color = "black"),  # Ensure axis text is also black
        axis.ticks = element_line(color = "black")) +  # Ensure axis ticks are black
...
global_kernel <- BAT::kernel.build(comm = global_comm, 
                                   trait = trait_space,
                                   method = "gaussian")
...
# Create the plot
kernel_plot <- ggplot() +
  geom_point(data = plot_data, aes(x = A1, y = A2, color = Density), alpha = 0.5) +
  scale_color_viridis_c() +
  #  geom_point(data = trait_data, aes(x = A1, y = A2), color = "red", size = 1, alpha = 0.7) +
  theme_minimal() +
  labs(title = "Global Kernel Density in Trait Space",
       x = "Trait Axis 1",
       y = "Trait Axis 2",
       color = "Density")
...
gam_plot <- ggplot() +
  ...
  scale_color_viridis_c(name = "Predicted % in ALL MPA") +
  theme_minimal() +
...
compute_phylo_signal <- function(tree, data) {
  ...
  print(paste("Number of species after matching and removing NAs:", nrow(matched_data)))
  
  # Prune the tree to match the available data
  pruned_tree <- keep.tip(tree, matched_data$species_modified)
  
  print(paste("Number of tips in pruned tree:", length(pruned_tree$tip.label)))
  
  # Compute phylogenetic signal using Pagel's lambda
  signal <- tryCatch({
    phylosig(pruned_tree, matched_data$percentage_in_ALL_MPA, method = "lambda", test = TRUE)
  }, error = function(e) {
    print(paste("Error in phylosig:", e$message))
    return(list(lambda = NA, P = NA))
  })
  
  # Return lambda, p-value, and number of species used
  return(c(lambda = signal$lambda, p_value = signal$P, n_species = nrow(matched_data)))
}

# Use the modified function in map_dfr
...
results %>%
  select(1:4) %>%
  kable(caption = "Phylogenetic Signal Results",
...
compute_phylo_signal <- function(tree, data) {
  ...
  # Compute phylogenetic signal using Pagel's lambda
  signal <- tryCatch({
    phylosig(pruned_tree, matched_data$percentage_in_NT_MPA, method = "lambda", test = TRUE)
  }, error = function(e) {
    print(paste("Error in phylosig:", e$message))
    return(list(lambda = NA, P = NA))
  })
