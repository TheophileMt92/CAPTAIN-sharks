...
significant_species <- function(comparison_data, mpa_type, z_threshold = 1.96) {
  ...
  kable_output <- kable(combined_table, format = "html", 
        col.names = c("Species", "Actual %", "Random %", "Difference", "SES"),  # Changed from "Z-score" to "SES"
        caption = paste("Top 10 significantly over/under-represented species in", mpa_type, "MPAs")) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                  full_width = FALSE) %>%
  ...
  save_kable(kable_output, 
             file = here::here("outputs", paste0("significant_species_", mpa_type, ".png")),
             zoom = 2)
...
#Overcome the infinite values in grid cells
# First, clean the SES values in comparison_NT
comparison_NT <- comparison_NT %>%
...
# Now recalculate the combined data with cleaned SES
...
mean_ses_per_cell_clean <- combined_data_clean %>%
  group_by(id, lon, lat) %>%
  summarise(mean_SES = mean(SES_clean, na.rm = TRUE)) %>%
  ungroup()

# Check the cleaned results
summary(mean_ses_per_cell_clean$mean_SES)

g_ses_2_clean <- ggplot() +
  geom_tile(data = mean_ses_per_cell_clean, aes(x = lon, y = lat, fill = mean_SES)) +
  geom_sf(data = land, fill = "darkgrey", color = NA) +
  scale_fill_gradient2(
    low = "#b2182b", mid = "#f0f0f0", high = "#2166ac", 
    midpoint = 0,
    limits = c(-3, 3),  # Now this makes sense with our -3 to +3 range
    oob = scales::squish
  ) +
...
print(ses_eco_map)
ggsave(here("Outputs", "ecoregion_SES_map_clean.png"), ses_eco_map, width = 12, height = 8, dpi = 300)
...
print(top_ses_plot)
ggsave(here("Outputs", "top_ecoregions_SES_clean.png"), top_ses_plot, width = 10, height = 8, dpi = 300)

print(bottom_ses_plot)
ggsave(here("Outputs", "bottom_ecoregions_SES_clean.png"), bottom_ses_plot, width = 10, height = 8, dpi = 300)
```
...
# Calculate clean SES for No-take (replacing the original SES calculation)
comparison_NT <- comparison_NT %>%
  mutate(SES_clean = case_when(
    sd_percentage == 0 & actual_percentage == mean_percentage ~ 0,
    sd_percentage == 0 & actual_percentage > mean_percentage ~ 3,   # Cap at +3
...
# Use the cleaned SES values in the combined data
combined_data_NT <- puvsp_long %>%
  left_join(comparison_NT %>% select(species, SES_clean), by = c("species" = "species"))

# Calculate mean SES per cell using cleaned values
mean_ses_per_cell_NT <- combined_data_NT %>%
  group_by(id, lon, lat) %>%
  summarise(mean_SES = mean(SES_clean, na.rm = TRUE)) %>%
  ungroup()
...
continental_plot_NT <- ggplot() +
  ...
  scale_color_gradient2(
    low = "#b2182b", mid = "#f0f0f0", high = "#2166ac", 
    midpoint = 0,
    limits = c(-3, 3),  # Set limits to match our cleaned SES range
...
ggsave(here::here("outputs","SES_map_highseas_NT_clean.png"), 
       high_seas_plot_NT,
       width = 10,
       height = 6,
       dpi = 300,
...
ggsave(
  filename = here::here("outputs", "updated_combined_plot_GAP_analysis_clean.png"),
...
# Transform percentage to (0,1) with boundary correction
results_IUCN <- results_IUCN %>%
  mutate(
    n_obs = n(),
    mpa_coverage_beta = (percentage_in_ALL_MPA/100 * (n_obs - 1) + 0.5) / n_obs
  )

# Fit beta regression
mpa_beta <- betareg(mpa_coverage_beta ~ iucn_GE, data = results_IUCN)
summary(mpa_beta)

# Get marginal effects
library(margins)
mpa_margins <- margins(mpa_beta)
summary(mpa_margins)

# Get predicted values by threat category
mpa_pred <- predict(mpa_beta, 
                    newdata = data.frame(iucn_GE = 1:5),
                    type = "response") * 100

print("Predicted MPA coverage by IUCN category:")
print(data.frame(
  Category = 1:5,
  IUCN = c("LC", "NT", "VU", "EN", "CR"),
  Predicted_Coverage = round(mpa_pred, 2)
))

# Average decrease per category
avg_decrease <- mean(diff(mpa_pred))
print(paste("Average decrease per threat category:", round(avg_decrease, 2), "%"))
...
# Modified plot with colored points instead of violins
...
final_plot_combined = ggarrange(
  rdplot_points_white,                       # Plot A
  nt_map_projected,
  stacked_bar_chart,                  # Plot A
  continental_plot_NT,
  nrow = 4,
...
# Define the output path using `here`
output_path <- here::here("outputs", "Fig_1_combined_points_2.png")
