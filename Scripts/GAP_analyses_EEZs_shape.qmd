...
cat("Global mean percentage of species ranges in No-Take MPAs:", 
    mean(grid_data$mean_NT_coverage, na.rm = TRUE), "\n")
cat("Global median percentage of species ranges in No-Take MPAs:", 
    median(grid_data$mean_NT_coverage, na.rm = TRUE), "\n")
cat("Standard deviation:", 
    sd(grid_data$mean_NT_coverage, na.rm = TRUE), "\n")
...
ggsave(here::here("NT_MPA_coverage_continental.png"), nt_map_projected, width = 10, height = 7, dpi = 300)

# Report continental waters statistics
cat("\nContinental Waters Statistics:\n")
cat("Mean percentage of species ranges in No-Take MPAs:", 
    mean(grid_data_continental$mean_NT_coverage, na.rm = TRUE), "\n")
cat("Max percentage of species ranges in No-Take MPAs:", 
    max(grid_data_continental$mean_NT_coverage, na.rm = TRUE), "\n")
cat("Min percentage of species ranges in No-Take MPAs:", 
    min(grid_data_continental$mean_NT_coverage, na.rm = TRUE), "\n")
cat("Median percentage of species ranges in No-Take MPAs:", 
...
# Save EEZ map
ggsave(here("Results", "eez_NT_coverage_map.png"), eez_map, width = 12, height = 8, dpi = 300)
...
# Save top EEZs plot
ggsave(here("Results", "top_eezs_NT_coverage.png"), top_eez_plot, width = 10, height = 8, dpi = 300)
```
...
# Helper function to find valid shift ranges
find_valid_shift_range <- function(patch_raster, valid_area_raster, buffer_cells = 2) {
  # Get extents
  patch_ext <- extent(patch_raster)
  valid_ext <- extent(valid_area_raster)
  
  # Get resolution
  res_x <- res(patch_raster)[1]
  res_y <- res(patch_raster)[2]
  
  # Calculate buffer
  buffer_x <- buffer_cells * res_x
  buffer_y <- buffer_cells * res_y
  
  # Calculate shift ranges with buffer
  min_shift_x <- valid_ext@xmin - patch_ext@xmin + buffer_x
  max_shift_x <- valid_ext@xmax - patch_ext@xmax - buffer_x
  min_shift_y <- valid_ext@ymin - patch_ext@ymin + buffer_y
  max_shift_y <- valid_ext@ymax - patch_ext@ymax - buffer_y
  
  return(list(
    x_range = c(min_shift_x, max_shift_x),
    y_range = c(min_shift_y, max_shift_y)
  ))
}

# Shape-preserving random MPA creation function
create_shape_preserving_random_mpa <- function(template_raster, marine_mask, eez_mask, max_attempts = 20) {
  # Convert to terra objects for processing
  template_terra <- rast(template_raster)
  marine_mask_terra <- rast(marine_mask)
  eez_mask_terra <- rast(eez_mask)
  
  # Initialize result
  random_raster <- template_terra * 0
  
  # Get unique country IDs
  country_ids <- unique(values(eez_mask_terra, na.rm = TRUE))
  country_ids <- country_ids[country_ids > 0 & !is.na(country_ids)]
  
  for (country_id in country_ids) {
    # Create country mask
    country_mask <- eez_mask_terra == country_id
    
    # Extract MPAs for this country
    country_mpas <- template_terra * country_mask
    country_mpas[country_mpas == 0] <- NA
    
    # Skip if no MPAs in this country
    if (all(is.na(values(country_mpas)))) next
    
    # Identify connected MPA patches
    mpa_patches <- patches(country_mpas, directions = 8, allowGaps = FALSE)
    
    if (is.null(mpa_patches) || all(is.na(values(mpa_patches)))) next
    
    # Get patch IDs
    patch_ids <- unique(values(mpa_patches, na.rm = TRUE))
    patch_ids <- patch_ids[!is.na(patch_ids)]
    
    # Create valid relocation area (marine areas within same EEZ)
    valid_area <- marine_mask_terra * country_mask
    
    for (patch_id in patch_ids) {
      # Extract individual MPA patch
      current_patch <- mpa_patches == patch_id
      current_patch[current_patch == 0] <- NA
      
      # Skip very small patches (less than 3 cells)
      if (sum(!is.na(values(current_patch))) < 3) next
      
      # Find valid shift ranges
      shift_ranges <- find_valid_shift_range(raster(current_patch), raster(valid_area))
      
      # Check if shifting is possible
      if (shift_ranges$x_range[2] <= shift_ranges$x_range[1] || 
          shift_ranges$y_range[2] <= shift_ranges$y_range[1]) {
        # If no valid shift range, try to place patch in a random valid location
        valid_cells <- which(!is.na(values(valid_area)) & values(valid_area) == 1)
        if (length(valid_cells) > 0) {
          # Convert current patch to points for relocation
          patch_cells <- which(!is.na(values(current_patch)))
          if (length(patch_cells) <= length(valid_cells)) {
            new_cells <- sample(valid_cells, size = length(patch_cells), replace = FALSE)
            random_raster[new_cells] <- 1
          }
        }
        next
      }
      
      # Attempt to find a valid shift
      success <- FALSE
      attempt <- 0
      ...
      while (!success && attempt < max_attempts) {
        ...
        shift_x <- runif(1, shift_ranges$x_range[1], shift_ranges$x_range[2])
        shift_y <- runif(1, shift_ranges$y_range[1], shift_ranges$y_range[2])
        
        # Apply shift
        shifted_patch <- shift(current_patch, dx = shift_x, dy = shift_y)
        
        # Mask to ensure it stays within valid marine areas
        shifted_patch_masked <- shifted_patch * valid_area
        
        # Check if the shifted patch maintains reasonable integrity
        original_cells <- sum(!is.na(values(current_patch)))
        shifted_cells <- sum(!is.na(values(shifted_patch_masked)) & values(shifted_patch_masked) == 1)
        
        # Accept if at least 70% of cells are preserved
        if (shifted_cells >= (original_cells * 0.7)) {
          # Add to result, avoiding overlap
          existing_mpas <- random_raster > 0
          non_overlapping <- shifted_patch_masked * (1 - existing_mpas)
          random_raster <- random_raster + non_overlapping
          success <- TRUE
        }
      }
      
      # If shifting failed, fall back to random placement
      if (!success) {
        valid_cells <- which(!is.na(values(valid_area)) & values(valid_area) == 1 & values(random_raster) == 0)
        patch_size <- sum(!is.na(values(current_patch)))
        
        if (length(valid_cells) >= patch_size) {
          new_cells <- sample(valid_cells, size = patch_size, replace = FALSE)
          random_raster[new_cells] <- 1
        }
      }
    }
  ...
  # Convert back to raster package format and ensure binary values
  result_raster <- raster(random_raster)
  result_raster[result_raster > 1] <- 1
  
  return(result_raster)
}

# Original random MPA function (for comparison)
...
# Main analysis function (updated to handle both methods)
run_random_mpa_analysis <- function(species_data, mpa_raster, marine_mask, eez_mask, 
                                   n_iterations = 100, use_shape_preserving = TRUE) {
  # Convert species dataframe to spatial points
  ...
  all_results <- pblapply(1:n_iterations, function(i) {
    ...
    if (use_shape_preserving) {
      random_mpa <- create_shape_preserving_random_mpa(mpa_raster, marine_mask, eez_mask)
    } else {
...
process_results <- function(results, mpa_type, method_suffix = "") {
  ...
  # Create filename - if no suffix, use original format
  if (method_suffix == "") {
    filename <- paste0("species_random_", mpa_type, "_coverage.csv")
  } else {
    filename <- paste0("species_random_", mpa_type, method_suffix, "_coverage.csv")
  }
  
  # Write results to CSV
  write.csv(results, here::here("Outputs", filename), row.names = FALSE)
...
# Run the analysis with both methods for comparison
cat("Running shape-preserving randomization analysis...\n")

# Shape-preserving method
tryCatch({
  random_results_ALL_shape <- run_random_mpa_analysis(
    puvsp_marine, mpa_ALL, marine_mask_resampled, eez_mask, 
    n_iterations = 20, use_shape_preserving = TRUE
  )
  cat("Shape-preserving analysis for ALL MPAs completed successfully.\n")
}, error = function(e) {
  cat(paste("Error in shape-preserving ALL MPAs analysis:", e$message, "\n"))
})

tryCatch({
  random_results_NT_shape <- run_random_mpa_analysis(
    puvsp_marine, mpa_NT, marine_mask_resampled, eez_mask, 
    n_iterations = 20, use_shape_preserving = TRUE
  )
  cat("Shape-preserving analysis for No-Take MPAs completed successfully.\n")
}, error = function(e) {
  cat(paste("Error in shape-preserving No-Take MPAs analysis:", e$message, "\n"))
})

# Original method for comparison
cat("Running original randomization analysis for comparison...\n")

tryCatch({
  random_results_ALL_original <- run_random_mpa_analysis(
    puvsp_marine, mpa_ALL, marine_mask_resampled, eez_mask, 
    n_iterations = 20, use_shape_preserving = FALSE
  )
  cat("Original analysis for ALL MPAs completed successfully.\n")
}, error = function(e) {
  cat(paste("Error in original ALL MPAs analysis:", e$message, "\n"))
})

tryCatch({
  random_results_NT_original <- run_random_mpa_analysis(
    puvsp_marine, mpa_NT, marine_mask_resampled, eez_mask, 
    n_iterations = 20, use_shape_preserving = FALSE
  )
  cat("Original analysis for No-Take MPAs completed successfully.\n")
}, error = function(e) {
  cat(paste("Error in original No-Take MPAs analysis:", e$message, "\n"))
})

# Process results for both methods
processed_results <- list()

# Shape-preserving results (with suffix for distinction)
if (exists("random_results_ALL_shape")) {
  processed_results[["ALL MPAs (Shape-preserving)"]] <- process_results(
    random_results_ALL_shape, "ALL_MPAs", "_shape_preserving"
  )
}

if (exists("random_results_NT_shape")) {
  processed_results[["No-take MPAs (Shape-preserving)"]] <- process_results(
    random_results_NT_shape, "No_take_MPAs", "_shape_preserving"
  )
}

# Original method results (using your original filename format)
if (exists("random_results_ALL_original")) {
  processed_results[["ALL MPAs (Original)"]] <- process_results(
    random_results_ALL_original, "ALL MPAs"  # This will create "species_random_ALL MPAs_coverage.csv"
  )
}

if (exists("random_results_NT_original")) {
  processed_results[["No-take MPAs (Original)"]] <- process_results(
    random_results_NT_original, "No-take MPAs"  # This will create "species_random_No-take MPAs_coverage.csv"
  )
}

# Print summary comparison
cat("\n=== ANALYSIS SUMMARY ===\n")
for (method_name in names(processed_results)) {
  cat(paste("\n", method_name, ":\n"))
  cat("Top 5 species by coverage:\n")
  print(head(processed_results[[method_name]]$top_10[, c("species", "mean_percentage")], 5))
  cat("Summary statistics:\n")
  print(processed_results[[method_name]]$summary_stats)
  cat(paste("Results saved to:", 
            list.files(here::here("Outputs"), 
                      pattern = paste0(".*", gsub("[^A-Za-z0-9_]", "_", method_name), ".*\\.csv"), 
                      full.names = FALSE)), "\n")
}

# Optional: Create comparison plots if both methods completed
if (exists("random_results_ALL_shape") && exists("random_results_ALL_original")) {
  cat("\nCreating comparison plots...\n")
  
  # Merge results for comparison
  comparison_data <- merge(
    random_results_ALL_shape[, c("species", "mean_percentage")],
    random_results_ALL_original[, c("species", "mean_percentage")],
    by = "species", suffixes = c("_shape", "_original")
  )
  
  # Create scatter plot
  pdf(here::here("Outputs", "method_comparison_plot.pdf"), width = 10, height = 8)
  plot(comparison_data$mean_percentage_original, comparison_data$mean_percentage_shape,
       xlab = "Original Method (% coverage)", ylab = "Shape-preserving Method (% coverage)",
       main = "Comparison of MPA Coverage Between Methods",
       pch = 16, col = "blue", alpha = 0.6)
  abline(0, 1, col = "red", lty = 2)  # 1:1 line
  dev.off()
  
  cat("Comparison plot saved to: method_comparison_plot.pdf\n")
}

random_results_ALL = random_results_ALL_shape 
random_results_NT = random_results_NT_shape 
...
random_ALL <- read.csv(here::here("Outputs", "species_random_ALL_MPAs_shape_preserving_coverage.csv"))
random_NT <- read.csv(here::here("Outputs", "species_random_No_take_MPAs_shape_preserving_coverage.csv"))
...
cat("Bottom 10 Ecoregions by Mean SES:\n")
...
ses_eco_map <- ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "Mean Standardized Effect Size (SES) by Marine Ecoregion",
    x = NULL, y = NULL
  ) +
  theme(
    legend.position = "bottom",
...
print(ses_eco_map)
ggsave(here("Outputs", "ecoregion_SES_map.png"), ses_eco_map, width = 12, height = 8, dpi = 300)
...
top_ses_plot <- ggplot(head(ecoregion_ses_stats, 20), 
  ...
  coord_flip() +
  labs(
    title = "Top 20 Marine Ecoregions by Mean SES",
    x = NULL,
    y = "Mean SES"
  ) +
  theme_minimal() +
...
bottom_ses_plot <- ggplot(tail(ecoregion_ses_stats, 20), 
  ...
  coord_flip() +
  labs(
    title = "Bottom 20 Marine Ecoregions by Mean SES",
    x = NULL,
    y = "Mean SES"
  ) +
  theme_minimal() +
...
print(top_ses_plot)
ggsave(here("Outputs", "top_ecoregions_SES.png"), top_ses_plot, width = 10, height = 8, dpi = 300)

print(bottom_ses_plot)
ggsave(here("Outputs", "bottom_ecoregions_SES.png"), bottom_ses_plot, width = 10, height = 8, dpi = 300)
```
...
# Read the datasets
comparison_ALL <- read.csv(here::here("Outputs", "species_random_ALL MPAs_coverage.csv"))
comparison_NT <- read.csv(here::here("Outputs", "species_random_No-take MPAs_coverage.csv"))

# For comparison with original method (if you want to compare)
comparison_ALL_original <- read.csv(here::here("Outputs", "species_random_ALL_MPAs_original_coverage.csv"))
comparison_NT_original <- read.csv(here::here("Outputs", "species_random_No_take_MPAs_original_coverage.csv"))

# Compare coverage between methods
shape_coverage <- sum(!is.na(comparison_ALL$mean_percentage))
original_coverage <- sum(!is.na(comparison_ALL_original$mean_percentage))

# Check which species have missing data
missing_species <- comparison_ALL %>% 
  filter(is.na(mean_percentage)) %>% 
  pull(species)

# Check spatial distribution of missing data
missing_locations <- combined_data %>% 
  filter(is.na(SES)) %>% 
  distinct(lon, lat)
```
...
# Grid of three GAP analysis plots for manuscript 
```{r}

library(ggpubr)

# Arrange the plots in a grid with the specified height ratios and add labels
grid_plot <- ggarrange(
  stacked_bar_chart,
  iucn_plot_NT,
  continental_plot_NT,
  ncol = 1,
  heights = c(0.25, 0.6, 1),
  labels = c("(A)", "(B)", "(C)"),
...
ggsave(here::here("outputs","combined_plot_GAP_analysis.png"), grid_plot, width = 8.5, height = 12, dpi = 300, bg="white")

library(ggpubr)
library(here)

# Create a new grid with only plots A and C
updated_grid_plot <- ggarrange(
  stacked_bar_chart,                  # Plot A
  continental_plot_NT,                # Plot C (renamed as B)
  ncol = 1,
  heights = c(0.25, 1),               # Adjust heights after removing Plot B
  labels = c("(A)", "(B)"),           # Update labels
  font.label = list(size = 16, face = "bold"),
  vjust = 1,
  hjust = -0.1
)

# Save the updated grid plot (A and C)
ggsave(
  filename = here::here("outputs", "updated_combined_plot_GAP_analysis.png"),
  plot = updated_grid_plot,
  width = 8.5,
  height = 9,                        # Adjust height to match the new layout
  dpi = 300,
  bg = "white"
)

# Save Plot B (iucn_plot_NT) separately
ggsave(
  filename = here::here("outputs", "plot_B_iucn.png"),
  plot = iucn_plot_NT,                # Save only Plot B
  width = 8.5,
  height = 6,                         # Set appropriate dimensions for Plot B
  dpi = 300,
  bg = "white"
)
...
model <- lm(percentage_in_ALL_MPA ~ iucn_GE, data=results_IUCN)
...
violin_plot_nt <- ggplot(results_IUCN_filtered, aes(x = factor(IUCN_status, levels = iucn_order), y = percentage_in_NT_MPA)) +
  ...
  geom_jitter(width = 0.1, size = 0.5, alpha = 0.5, color = "darkgray") +
  geom_boxplot(width = 0.1, fill = "white", color = "black", outlier.shape = NA, alpha = 0.8) +
  labs(x = "IUCN Red List threat status", y = "(%) Range within no-take MPAs") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none") +
  scale_x_discrete(limits = iucn_order) +
  scale_y_continuous(transform = "sqrt", limits = c(0,80)) +
  scale_fill_manual(values = iucn_colors) +
  scale_color_manual(values = iucn_colors)
...
ggsave(here::here("violin_plots_notake_protection.png"), violin_plot_nt, width = 6, height = 6, units = "in")
...
# Create new dataset with species IDs for both datasets
# Assuming the order is preserved (species are in the same order in both files)
if(nrow(protected_fractions) == nrow(shark_metrics)) {
  # Add row numbers as common identifiers
  protected_fractions$ID <- 1:nrow(protected_fractions)
  shark_metrics$ID <- 1:nrow(shark_metrics)
  ...
  # Check the first few rows to confirm the join worked
  head(combined_data)
  
  # Reshape data for plotting
  # Convert to long format for faceting by index
  combined_long <- combined_data %>%
    pivot_longer(
      cols = c(IUCN_captain, FUSE_captain, EDGE2_captain),
      names_to = "index_captain",
      values_to = "protection_fraction"
    ) %>%
    # Remove "_captain" suffix
    mutate(index = gsub("_captain", "", index_captain)) %>%
    # Convert protection fraction to percentage
    mutate(protection_percentage = protection_fraction * 100)
  ...
  # Create the plot
  iucn_plot <- combined_long %>%
    mutate(
      # Convert IUCN numeric values to factor with labels
      IUCN_status = factor(IUCN_original, levels = as.numeric(names(iucn_labels)), labels = iucn_labels)
    ) %>%
    ...
    geom_boxplot(width = 0.1, 
                 ...
                 outlier.shape = NA, 
                 alpha = 0.8) +
    facet_wrap(~index, ncol = 3, dir = "h") +
    labs(title = "Range Protection by IUCN Status for Different Conservation Indices",
         x = "IUCN Red List threat status", 
         y = "Range protected (%)") +
    theme_minimal() +
    theme(
  ...
  # Save the plot
  ggsave(here::here("outputs", "protection_by_iucn_status.png"), 
         iucn_plot, width = 12, height = 8, dpi = 300)
  
  # Create plots comparing original metrics vs protected fractions
  
  # For IUCN
  iucn_comparison <- combined_data %>%
    mutate(
      IUCN_status = factor(IUCN_original, levels = as.numeric(names(iucn_labels)), labels = iucn_labels),
    ...
    geom_boxplot(width = 0.1, 
                 ...
                 outlier.shape = NA, 
                 alpha = 0.8) +
    labs(title = "IUCN Priority Index: Protection by IUCN Status",
         x = "IUCN Red List threat status", 
         y = "Range protected (%)") +
    theme_minimal() +
    theme(
  ...
  fuse_comparison <- combined_data %>%
    ...
    geom_boxplot(width = 0.1, 
                 ...
                 outlier.shape = NA, 
                 alpha = 0.8) +
    labs(title = "FUSE Priority Index: Protection by Original FUSE Score",
         x = "Original FUSE Score", 
         y = "Range protected (%)") +
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5),
  ...
  # For EDGE2
  edge2_comparison <- combined_data %>%
    mutate(
    ...
    geom_boxplot(width = 0.1, 
                 ...
                 outlier.shape = NA, 
                 alpha = 0.8) +
    labs(title = "EDGE2 Priority Index: Protection by Original EDGE2 Score",
         x = "Original EDGE2 Score", 
         y = "Range protected (%)") +
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5),
  ...
  print(iucn_comparison)
  print(fuse_comparison)
  print(edge2_comparison)
  
  # Save individual plots
  ggsave(here::here("outputs", "iucn_protection_comparison.png"), 
         iucn_comparison, width = 10, height = 6, dpi = 300)
  ggsave(here::here("outputs", "fuse_protection_comparison.png"), 
         fuse_comparison, width = 10, height = 6, dpi = 300)
  ggsave(here::here("outputs", "edge2_protection_comparison.png"), 
         edge2_comparison, width = 10, height = 6, dpi = 300)
  
  # Calculate summary statistics by category
  summary_stats <- combined_long %>%
    mutate(
      IUCN_status = factor(IUCN_original, levels = as.numeric(names(iucn_labels)), labels = iucn_labels),
      protection_percentage = protection_fraction * 100
    ) %>%
    group_by(index, IUCN_status) %>%
    summarise(
      mean_protection = mean(protection_percentage, na.rm = TRUE),
      median_protection = median(protection_percentage, na.rm = TRUE),
      sd_protection = sd(protection_percentage, na.rm = TRUE),
      n_species = n(),
      .groups = "drop"
    )
  ...
  write.csv(summary_stats, here::here("outputs", "protection_summary_by_category.csv"), row.names = FALSE)
...
### Ridgeline chart : no-take ms chart
...
# Modified plot with mean dots and SD whiskers bounded at 0
rdplot = ggplot(notake_mpa_iucn, 
  ...
  geom_jitter(width = 0.1, 
              ...
              alpha = 0.5, 
              color = "darkgray") +
  # Replace boxplot with stat_summary for mean and SD
  ...
  stat_summary(fun.data = function(x) {
                  ...
                  return(data.frame(y = mean_val, 
                                   ymin = max(0, mean_val - sd_val), # Ensure lower bound is not below 0
  ...
  scale_y_sqrt(limits = c(0, 50),
                    breaks = seq(0, 50, 10), # Ensure tick marks are at increments of 10
    labels = c("0", "10", "20", "30", "40", "")) # Remove the text label for 50
rdplot
...
# Combine the two plots into a grid with two rows and one column
# Adjust the relative heights using the `heights` argument, and add labels
final_plot <- ggarrange(
  rdplot_white,                       # Plot A
  nt_map_projected,                   # Plot B
  ncol = 1,                           # 1 column
  nrow = 2,                           # 2 rows
  heights = c(1, 1.5),                # Relative heights (less height for A)
...
# Calculate mean fishing hours by country (sovereign state)
# Load necessary libraries
library(sf)
library(dplyr)
library(here)
...
# Fix the geometry issues in the EEZ shapefile
eez_fixed <- eez %>%
  st_make_valid() %>%  # Fix any invalid geometries
  select(SOVEREIGN1)   # Select only the column we need

# Check if geometries are now valid
if(all(st_is_valid(eez_fixed))) {
  print("All EEZ geometries are now valid")
} else {
  print("Some EEZ geometries are still invalid")
}

# Use a different spatial joining approach that's more robust to topology errors
# Method 1: Try regular spatial join with fixed geometries
fishing_with_country <- st_join(fishing_sf, eez_fixed)

# If that still fails, try alternative approaches:
# Method 2: Point-in-polygon operation that's more tolerant
if (!exists("fishing_with_country")) {
  fishing_with_country <- fishing_sf
  fishing_with_country$SOVEREIGN1 <- NA
  
  # Loop through each EEZ polygon (this is slower but more robust)
  for(i in 1:nrow(eez_fixed)) {
    current_eez <- eez_fixed[i,]
    # Find points in this EEZ
    points_in_eez <- st_intersects(fishing_sf, current_eez, sparse = TRUE)
    # Get indices of points in this EEZ
    indices <- unlist(points_in_eez)
    # Assign SOVEREIGN1 value to these points
    if(length(indices) > 0) {
      fishing_with_country$SOVEREIGN1[indices] <- current_eez$SOVEREIGN1
    }
  }
}

# Method 3: If still failing, try simplifying the EEZ geometries
if (!exists("fishing_with_country")) {
  eez_simplified <- eez_fixed %>%
    st_simplify(dTolerance = 0.01)  # Simplify geometry (adjust tolerance as needed)
  
  fishing_with_country <- st_join(fishing_sf, eez_simplified)
}

# Label points not in any EEZ as "High Seas"
fishing_with_country <- fishing_with_country %>%
  mutate(SOVEREIGN1 = ifelse(is.na(SOVEREIGN1), "High Seas", SOVEREIGN1))

# Calculate statistics by country with 5th and 95th percentiles
country_statistics <- fishing_with_country %>%
  st_drop_geometry() %>%
  group_by(SOVEREIGN1) %>%
  summarize(
...
# Top 20 countries by mean fishing hours
top_countries <- country_statistics %>%
  arrange(desc(mean_fishing_hours)) %>%
  head(20)

# Visualization of top countries
ggplot(top_countries, aes(x = reorder(SOVEREIGN1, mean_fishing_hours), y = mean_fishing_hours)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_errorbar(aes(ymin = q05, ymax = q95), width = 0.2) +
  geom_point(aes(y = median_fishing_hours), color = "red", size = 2) +
  labs(title = "Top 20 Countries by Mean Fishing Hours in EEZs",
       subtitle = "Bars: mean values | Red points: median values | Error bars: 5th-95th percentiles",
       x = "Country",
       y = "Fishing Hours") +
  coord_flip() +
  theme_minimal()
