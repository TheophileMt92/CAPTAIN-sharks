...
# Optional: Create a visualization of the randomization test results
library(ggplot2)

# Example for FUSE vs EDGE2 (you can repeat for others)
hist_data <- data.frame(random_overlap = test_fuse_edge2$random_overlaps)

randomization_plot <- ggplot(hist_data, aes(x = random_overlap)) +
  geom_histogram(bins = 50, fill = "lightblue", alpha = 0.7, color = "black") +
  geom_vline(xintercept = test_fuse_edge2$observed_percentage, 
             color = "red", linetype = "dashed", size = 1) +
  geom_vline(xintercept = test_fuse_edge2$expected_percentage, 
             color = "blue", linetype = "solid", size = 1) +
  labs(title = "Randomization Test: FUSE vs EDGE2 Congruence",
       subtitle = paste("Observed overlap:", round(test_fuse_edge2$observed_percentage, 2), 
                        "%, P-value:", test_fuse_edge2$p_value),
       x = "Overlap Percentage (%)",
       y = "Frequency") +
  theme_minimal() +
  annotate("text", x = test_fuse_edge2$observed_percentage, 
           y = Inf, label = "Observed", vjust = 2, color = "red") +
  annotate("text", x = test_fuse_edge2$expected_percentage, 
           y = Inf, label = "Expected", vjust = 2, color = "blue")

print(randomization_plot)
```
